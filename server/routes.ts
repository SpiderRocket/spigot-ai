...
Generate the complete Main.java class code. If commands are enabled, implement the onCommand method with the logic for the special requirements. If events are enabled, implement any necessary @EventHandler methods.`;
  ...
  // Download endpoint - creates ZIP file
  app.post("/api/download-plugin", async (req, res) => {
    try {
      const { files, pluginName } = req.body;
...
function generatePluginYml(data: z.infer<typeof generatePluginSchema>): string {
...
function generatePomXml(data: z.infer<typeof generatePluginSchema>, libs: { name: string; maven: string; repository?: string }[]): string {
  ...
  // Add library repositories and dependencies
...
function generateBuildGradle(data: z.infer<typeof generatePluginSchema>, libs: { name: string; maven: string; repository?: string }[]): string {
...
function generateConfigYml(data: z.infer<typeof generatePluginSchema>): string {
